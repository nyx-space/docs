...
3.  **Serialization**: All queries can be serialized to **S-expressions** (symbolic expressions). This allows you to define a query in Python, send it to a remote Rust worker, and execute it safely without allowing arbitrary code execution.
...
-   **Condition**: A threshold or extrema (e.g., `> 5.0 deg`, `Maximum`, `Minimum`).
...
1.  **Search**: It scans the time domain.
